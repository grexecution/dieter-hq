...
export function InboxItemCard({ 
  ...
  const handleDragEnd = (_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    if (shouldSwipeRight && item.status !== "pending") {
      // Swipe right = mark as pending (bring back)
  ...
  const senderInitials = (item.senderName || item.sender)
    .split(" ")
    .map(n => n[0])
    .join("")
    .slice(0, 2)
  ...
  return (
    <div ref={constraintsRef} className="relative overflow-hidden rounded-lg">
      ...
      <div className="absolute inset-0 flex">
        ...
        <motion.div 
          className="flex-1 flex items-center justify-start pl-4 bg-green-500"
          style={{ opacity: leftActionOpacity }}
        >
          <motion.div style={{ scale: leftActionScale }} className="text-white">
        ...
        <motion.div 
          className="flex-1 flex items-center justify-end pr-4 bg-zinc-500"
          style={{ opacity: rightActionOpacity }}
        >
          <motion.div style={{ scale: rightActionScale }} className="text-white">
